...
function GameNavigator({move, historyLen, cells, navigateHandler}) {
	const prevButton = (
		<button class={"navigator" + (move > 0 ? "" : " disabled")} onClick={() => navigateHandler(Math.max(0, move - 1))}>{"<"}</button>
	);
	const winner = calculateWinner(cells);
	const statusText = winner == null ? (move % 2 ? "✘" : "✔") : ("⚑" + (move % 2 ? "✔" : "✘") + "⚑");
	const statusCell = (
		<button class={"navigator " + (winner == null ? "on-progress" : "game-over")} onClick={() => navigateHandler(0)}>{statusText}</button>
	);
	const nextButton = (
